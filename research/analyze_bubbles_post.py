...
for trumps in categories:
    ...
    for i, row in grouped.iterrows():
        if row['Count'] > 50: # Only label significant bubbles
            plt.text(
                row['PCntJ'], 
                row['PSafeFulls'], 
                f"{row['MeanWinRate']:.0%}\n(n={row['Count']})", 
    ...
    try:
        ...
        fig = px.scatter(
            ...
            title=f"Post-Discard Strength: {title_suffix}<br><sup>Win Prob based on Jacks vs. Standing Fulls. Size=Frequency.</sup>",
